...
<aside
    class="dashboard-sidebar fixed inset-y-0 left-0 z-40 flex w-64 -translate-x-full flex-col border-r border-[#151515] bg-[#050505] text-white transition-transform duration-300 lg:translate-x-0"
    data-sidebar>
    <div class="flex items-center justify-between border-b border-[#151515] px-6 py-7">
        ...
        <button type="button"
            class="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 bg-white/5 text-white/70 lg:hidden"
            ...
            <div class="space-y-2">
                @foreach ($group['links'] as $link)
                    ...
                    @if ($hasChildren)
                        <div>
                            <button type="button"
                                class="sidebar-trigger flex w-full items-center justify-between rounded-lg px-4 py-3 text-left text-[1.05rem] font-medium transition-colors duration-200 {{ $isExpanded ? 'is-active bg-white/8 text-white' : 'text-white hover:bg-white/5' }}"
                                data-submenu-toggle aria-expanded="{{ $isExpanded ? 'true' : 'false' }}">
                                <span>{{ $link['label'] }}</span>
                                <svg class="submenu-arrow h-4 w-4 text-white transition-transform duration-200"
                            ...
                            <div class="submenu overflow-hidden {{ $isExpanded ? 'is-open' : '' }}" data-submenu>
                                <div class="mt-1 space-y-1">
                                    @foreach ($link['children'] as $child)
                                        <a href="{{ $child['route'] }}"
                                            class="flex items-center gap-3 rounded-lg px-4 py-3 text-[1rem] transition-colors duration-200 {{ !empty($child['active']) ? 'bg-[#03282d] text-cyan-500' : 'text-white/80 hover:bg-white/5 hover:text-white' }}">
                    ...
                    @else
                        <a href="{{ $link['route'] }}"
                            class="flex items-center gap-3 rounded-lg px-4 py-3 text-[1.05rem] font-medium transition-all duration-200 {{ $isActive ? 'bg-[#03282d] text-cyan-500' : 'text-white hover:bg-white/5' }}">
